...
func (p *Producer) Close() error {
	ch := make(chan error) // this is available of any var which type is Producer
	p.quit <- ch
	return <-ch // return err or nil if channal is closed successfuly, but wont be nil if something goes wrong and channal isn't close
}
...
func makePizza(pizzaNumber int) *PizzaOrder {
	...
	// u slucaju da smo presli limit broja pica vracamo pointer na strukturu sa pizzaNumber = 11 u nasem slucaju, posto je limit postavljen na 10
	return &PizzaOrder{
		pizzaNumber: pizzaNumber,
	}
}
	...
	for {
		...
		if currentPizza != nil {
			...
			case quitChan := <-pizzaMaker.quit:
				...
				close(quitChan)
				return // bitno zbog izlaska iz ove funkcije tj. gorutine
			}
		}
	}
}
...
func main() {
	...
	for i := range pizzaJob.data { // prolazimo kroz channal
