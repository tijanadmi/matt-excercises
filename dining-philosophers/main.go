...
type Philosopher struct {
	name      string
	rightFork int // moramo koristiti sync.Mutax kada filozof koristi levu i desnu viljusku
	leftFork  int // moramo koristiti sync.Mutax kada filozof koristi levu i desnu viljusku
}
...
func dine() {
	...
	// wait group.
	wg := &sync.WaitGroup{}
	wg.Add(len(philosophers)) // it is done kada svi filozofi ustanu sa stola, kada je vrednost 0 ona je zavrsila

	// We want everyone to be seated before they start eating, so create a WaitGroup for that, and set it to 5.
	seated := &sync.WaitGroup{}
	seated.Add(len(philosophers)) // it is done kada svi filozofi sede za stolom, kada je vrednost 5 ona je zavrsila
	...
	var forks = make(map[int]*sync.Mutex) // mapa svih 5 viljusaka, vrednost u mapi je pointer ka mutaxu
